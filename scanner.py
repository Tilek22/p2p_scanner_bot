...
TRC20_FEE = 1.0
MIN_PROFIT_THRESHOLD = 0.8  # Минимальная прибыль в %

def get_binance_usdt_usd():
...
def get_okx_usdt_usd():
    url = "https://www.okx.com/v3/c2c/otc-ticker?quoteCurrency=USD&baseCurrency=USDT&side=sell&paymentMethod=&userType=all"
    try:
        r = requests.get(url)
        data = r.json()
...
def compare_all_exchanges():
    prices = {
        "Binance": get_binance_usdt_usd(),
        "OKX": get_okx_usdt_usd(),
        "Bybit": get_bybit_usdt_usd()
    }
    ...
    if not results:
        return ["😕 No profitable routes found (≥ 0.8%)"]
